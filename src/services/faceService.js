...
async function pollPrediction(predictionId, replicateAuthConfig, timeoutMs = 120000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    log('Replicate face service status:', prediction.status);
...
export async function swapFaces(sourceImageUrl, targetImageUrl, model = 'deepfaceswap/deepfaceswap', replicateAuthConfig = null) {
  if (!sourceImageUrl || typeof sourceImageUrl !== 'string') {
    throw new Error('sourceImageUrl is required and must be a string');
  }
  if (!targetImageUrl || typeof targetImageUrl !== 'string') {
    throw new Error('targetImageUrl is required and must be a string');
  }

  await globalRateLimiter.check('swapFaces');
...
export async function changeFaceExpression(imageUrl, expression, model = 'sberbank-ai/face-expression-swap', replicateAuthConfig = null) {
  ...
  await globalRateLimiter.check('changeFaceExpression');
